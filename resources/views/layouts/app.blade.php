...
<head>
    ...
    <title>@yield('title', 'Ruislip Mobile')</title>
    ...
    <script>window.currencySymbol = @json($currency ?? '£');</script>
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<body class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <main class="flex-1">
        ...
        @endif
        @if(session('error'))
            <div class="bg-red-100 text-red-800 text-sm text-center py-2">{{ session('error') }}</div>
        @endif
    ...
    @php $waNumber = preg_replace('/[^0-9]/', '', $whatsappNumber); $waUrl = 'https://wa.me/' . $waNumber; @endphp
    <a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer" class="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-lg hover:bg-[#20bd5a] hover:scale-110 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-[#25D366]/50" aria-label="Chat on WhatsApp">
